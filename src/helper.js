...
export const getRandomComputerMove = (gameBoard) => {
    ...
    for (let i = 0; i < gameBoard.length; i++) {
        if (gameBoard[i] === 0) validMove.push(i);
    }
    let move = Math.floor(Math.random() * validMove.length);
    return validMove[move];
}

const getPosition = (gameBoard, moveChecks) => {
    for (let i = 0; i < moveChecks.length; i++) {
        for (let j = 0; j < moveChecks[i].max; j = j + moveChecks[i].step) {
            let played = 
                gameBoard[j + moveChecks[i].indexes[0]].toString() +
